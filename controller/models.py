...
class Doctor:
    ...
    def list():
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            return cursor.fetchall()  # Retornar todos los doctores
...
class Patient:
    ...
    def save(self):
        ...
        with sqlite3.connect('database.db') as conn:  # Conectar a la base de datos
            ...
            conn.commit()  # Guardar los cambios en la base de datos
# Clase Cita: Representa una cita en el sistema
class Cita:
    def __init__(self, doctor_id, patient_id, fecha, motivo):
        # Constructor de la clase Appointment
        self.doctor_id = doctor_id  # ID del doctor asociado
        self.patient_id = patient_id  # ID del paciente asociado
        self.fecha = fecha  # Fecha y hora de la cita
    ...
    def find_by_id(appointment_id):
        # Método estático para buscar una cita por su ID
        with sqlite3.connect('database.db') as conn:  # Conectar a la base de datos
            cursor = conn.cursor()
            # Buscar la cita en la tabla 'appointments'
            cursor.execute('SELECT * FROM appointments WHERE id = ?', (appointment_id,))
            return cursor.fetchone()  # Retornar la fila de la cita (o None si no existe)
    def list():
        # Método para listar todas las citas
        with sqlite3.connect('database.db') as conn:
            cursor = conn.cursor() # Conectar a la base de datos
            cursor.execute('SELECT * FROM citas')
            return cursor.fetchall() # Retornar todas las citas